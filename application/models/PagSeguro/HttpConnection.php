<?php
class Application_PagSeguro_HttpConnection {
    ...
    public function postPagamento($url, $data = null, $timeout = null, $charset = null) {
        return $this->curlConnectionPagamento('POST', $url, $data, $timeout, $charset);
    }
    ...
    private function curlConnectionPagamento($method, $url, $data = null, $timeout = 20, $charset = 'ISO-8859-1') {

        if (strtoupper($method) === 'POST') {
            $url = $url."?email=cabral@scrapi.com.br&token=BD572FD0880A4646AF6F6396FFA95099";
            $postFields = $data;
            $contentLength = "Content-length: " . strlen($postFields);
            $methodOptions = array(
                CURLOPT_POST => true,
